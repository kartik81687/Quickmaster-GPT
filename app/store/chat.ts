  ...
  persist(
    (set, get) => ({
      ...
      async onUserInput(
        ...
        console.log("ducduckgo res: ", sources);
        const filteredSources = duckduckgo ? sources : [];
        ...
        const systemInfo = createMessage({
          role: "system",
          content:
            `IMPORTANT: You are a virtual assistant powered by the ${
              modelConfig.model
            } model, now time is ${new Date().toLocaleString()}}` +
            (duckduckgo
              ? `Use the sources to provide an accurate response. Respond in markdown format. Cite the sources you used as [1](link), etc, as you use them. Maximum 4 sentences.`
              : ""),
          id: botMessage.id! + 1,
        });
        ...
        if (barding) {
                ...
                await fetch(
                  "https://sa-translate.p.rapidapi.com/translate/text",
                  {
                    ...
                    headers: {
                      ...
                      "X-RapidAPI-Key":
                        "6136174245msh0c925e32b130600p1cf50cjsn9c31f2dde361",
          ...
          try {
            const { body, ok } = await fetch("/api/anthropic", {
              method: "POST",
              body: JSON.stringify({
